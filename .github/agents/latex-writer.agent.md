...
5. 按知识点顺序，为当前 subsection 生成或更新 LaTeX 内容；概念型知识点同样应写入 LaTeX。
6. 写作时严格复用 `macros.tex` 中已有宏，禁止同一符号在不同地方使用不一致写法；只有在确实缺少符号宏时，才向 `macros.tex` 补充新宏。
7. 对不同类型知识点，严格使用 `packages.tex` 中已定义的数学环境，如 `definition`、`axiom`、`theorem`、`proposition`、`lemma`、`corollary`、`example`、`remark`、`note`、`proof` 与 `important`。
8. 对重要的引理、命题、定理、推论和例子，优先参考 Lean4 结果，写出自然语言与一阶逻辑风格证明。
9. 对暂时无法完成的证明，在 LaTeX 文件中插入清晰的 TODO 标记，并同步写入 `latex/latex_todo.yml`。
10. 如有必要，更新最近的包含文件或入口文件，确保 `latex/main.tex` 仍然可编译。
11. 执行 `latexmk -r latexmkrc -xelatex main.tex` 作为强制验证；只有验证通过的内容，才能回写对应知识点 YAML 中的 `latex_done: true`。
...
- `latexmk -r latexmkrc -xelatex main.tex` 无法通过且错误无法在本轮局部修复
