...
4. 按“公理/定义在前，命题性条目在后”的顺序生成或更新 Lean 声明。
5. 优先使用 Term 风格；对复杂长证明允许使用 Tactic 风格。
6. 在关键证明步骤用中文注释说明思路。
7. 对暂时无法完成的条目，在 Lean 文件中保留清晰 TODO 标记，并同步写入 `lean/lean_todo.yml`。
8. 执行 Lean 验证；只有验证通过的条目，才能回写对应知识点 YAML 中的 `lean_done: true`。
